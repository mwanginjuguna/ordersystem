    ...
    <BaseLayout>
        <template #header>
            ...
            <p class="justify-self-center text-center text-xs text-gray-500 p-4 border-b border-purple-500">
                <span class="font-bold">About: </span>Orders with disputes.
            </p>
        </template>
        <section>
            <OrdersCard>
                    ...
                    <div>
                        <div class="p-3 lg:py-10 lg:px-8 m-2 md:grid md:grid-cols-3 lg:mx-8 lg:my-4">
                            ...
                            <div class="md:col-span-2 lg:col-span-3 text-sm md:text-md space-y-6">
                                <div v-for="disputed in disputedOrders"
                                     class="flex justify-between md:max-w-2/3 lg:w-full mx-auto border-b border-indigo-400">
                                    <div>
                                        <p>Order Topic: {{ disputed.title }}</p>
                                        <p class="text-xs">Order Id: <span>#{{ disputed.id }}</span></p>
                                    </div>
                                    <div class="p-2">
                                        <Link :href="route('orders.show', disputed.id)"
                                              class="text-sky-400 hover:text-sky-500 underline mx-2">
                                            view
                                        </Link>
                                        <Link :href="route('orders.delete', disputed.id)" class="text-red-400 hover:text-red-500 underline mx-2">Delete</Link>
                                    </div>
                                </div>
                            </div>
